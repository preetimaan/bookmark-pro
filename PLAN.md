...
- **Import / Export (Group 5)** — Export as HTML (Netscape Bookmark File format) or as JSON (full tree + tags keyed by bookmark id). Import from HTML (parse DL/DT/A and H3+DL) or from JSON (create tree under selected folder, map old ids to new ids, apply tags). **Import from backup:** after file selection, a modal lists “Entire file” plus every folder in the backup (by path); user can import only a chosen folder into the selected destination. Import target: selected folder or Bookmarks bar. `dateAdded` is not passed to `chrome.bookmarks.create` (API does not support it); imported items get current creation time.
